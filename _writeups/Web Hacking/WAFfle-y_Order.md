...
image: /assets/images/Screenshots/WAFfle-y_Order/waffle-y_order_thumbnail.png
twitter_text: HackTheBox &quot;WAFfle-y_Order&quot; wirteup is now available!
description:
---

{% include header.html %}
![thumbnail.png]({{page.screenshots}}waffle-y_order_thumbnail.png)
...
**WAFfle-y Order** is a medium-difficulty web challenge from **HackTheBox** that involves attacking a custom application vulnerable to **XXE Injection** and **PHP arbitrary deserialization** **bypassing two different WAF regexes**.

## Improved skills
- **Code review** for PHP applications
- Exploit **PHP deserialization** vulnerabilities
- Exploit **XML External Entity Injection** vulnerabilities
- Bypass **regex-based WAF** using **parser differentials**
...
![Home page]({{page.screenshots}}WAFfle-y_Order_bc7ec381b01f4d9187fad6632cc87721.png)
...
If the regex matches some PHP magic methods [^magic] - so the one starting with `__` - then the application returns an error, otherwise, it sets up the user object and then returns the message that pops out when submitting the order. 

[^magic]: [https://www.php.net/manual/en/language.oop5.magic.php](https://www.php.net/manual/en/language.oop5.magic.php)
...
[^construct]: [https://www.php.net/manual/en/language.oop5.decon.php#object.construct](https://www.php.net/manual/en/language.oop5.decon.php#object.construct)
[^wakeup]: [https://www.php.net/manual/en/language.oop5.magic.php#object.wakeup](https://www.php.net/manual/en/language.oop5.magic.php#object.wakeup)
...
- the same class also uses the PHP Complex Syntax [^complex], that in some cases can lead to RCE, 
...
[^complex]: [https://www.php.net/manual/en/language.types.string.php#language.types.string.parsing.complex](https://www.php.net/manual/en/language.types.string.php#language.types.string.parsing.complex)
...
After some research I found this wonderful writeup [^writeup] from MegadodoPubblications talking about a Remote Code Execution in Composr. Although the target is different than ours, going through the article I found a really interesting thing. The author had to bypass the same regex we’re facing.

[^writeup]: [https://github.com/MegadodoPublications/exploits/blob/master/composr.md](https://github.com/MegadodoPublications/exploits/blob/master/composr.md)
...
[^Spl]: [https://github.com/MegadodoPublications/exploits/blob/master/composr.md#the-moderately-cool-way-that-works](https://github.com/MegadodoPublications/exploits/blob/master/composr.md#the-moderately-cool-way-that-works)
...
![]({{page.screenshots}}waffle-y_regex101_1.png)
...
Unfortunately, the data contained in our XML is not reflected anywhere in the application. This means that we cannot exfiltrate data by reading it through the app, but we must necessarily use OAST [^oast] techniques. Using external entities we can force the web application to perform an **HTTP request to our server**, sending us any data we extracted using other entities.

[^oast]: [https://portswigger.net/burp/application-security-testing/oast](https://portswigger.net/burp/application-security-testing/oast)
...
To concatenate multiple entities inside the Document Type Definition we can use parameter entities [^param-ent], special XML entities that can only be referenced elsewhere within the DTD. They use the “%” character before their name but they can be referenced like traditional XML entities. We can place the malicious payload inside an external DTD and then download it using a simple parameter entity.

[^param-ent]: [https://www.w3resource.com/xml/parameter-entities.php](https://www.w3resource.com/xml/parameter-entities.php)
...
![]({{page.screenshots}}waffle-y_regex101_2.png)

Unfortunately, it does, but as we saw from the Portswigger site [^obf1] and also from this other post [^obf2], we can use **entities inside XML strings** to try to find a workaround for restricted XXE.

[^obf1]: [https://portswigger.net/web-security/essential-skills/obfuscating-attacks-using-encodings](https://portswigger.net/web-security/essential-skills/obfuscating-attacks-using-encodings)
[^obf2]: [https://github.com/Ambrotd/XXE-Notes](https://github.com/Ambrotd/XXE-Notes)
...
![]({{page.screenshots}}waffle-y_regex101_3.png)

![WAFfle-y_Order_bc7ec381b01f4d9187fad6632cc87721]({{page.screenshots}}WAFfle-y_Order_bc7ec381b01f4d9187fad6632cc87721_1.png)
...
![WAFfle-y_Order_bc7ec381b01f4d9187fad6632cc87721]({{page.screenshots}}WAFfle-y_Order_bc7ec381b01f4d9187fad6632cc87721_2.png)
...
After having completed the challenge, thanks to SSCx7832 [^ssc] I discovered that also the second bypass technique was working and could be used as well to evade the regex checking for malicious PHP objects.

[^ssc]: [https://sys3cybersecurity.netlify.app/htb-waffle-y-order/](https://sys3cybersecurity.netlify.app/htb-waffle-y-order/)
...
![]({{page.screenshots}}waffle-y_regex101_4.png)
